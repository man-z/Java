...
public class SplayTree<T extends Comparable<T>>{
    ...
    private SplayTreeNode<T> splay(SplayTreeNode<T> tree, T key) {
        ...
        for (;;) {
            ...
            if (cmp < 0) {
                ...
                if (key.compareTo(tree.left.key) < 0) {//小于左边的树，单旋转
    ...
    private SplayTreeNode<T> insert(SplayTreeNode<T> tree, SplayTreeNode<T> z) {
        ...
        // 查找z的插入位置；循环树，一直找到z插入的位置，当树等于null是说明找到z插入的位置
        //循环到最后一次后，x树为空，y是z插入的位置
        while (x != null) {
            y = x;
    ...
    private static final int arr[] = {10,50,40,30,20,60,5,6};
