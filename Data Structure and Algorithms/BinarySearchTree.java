...
public class BinarySearchTree {
    ...
    public void binarySearchTree(){
        ...
        backtrace(s, n, 1, n, 0, "0");
    }

    public void optimalBinarySearchTree1(float[] a,float[] b,float[][] m,int[][] s,float[][] w){
        int n=a.length-1;
        for(int i=0;i<=n;i++){//初始化构造无内部节点的情况
            w[i+1][i]=a[i];
            m[i+1][i]=0;
    ...
    public void backtrace1(int[][] s,int n,int i,int j,int p,String str){
        int k=s[i][j];
        if(k>0){
            if(p==0){
                System.out.println("root is:"+k);
    ...
    public void binarySearchTree1(){
        ...
        int[][] s=new int[n+2][n+2];//因为w[i+1][i]存在，i最大为n，即可以从0.....n,n+1,共n+2个
        float[][] w=new float[n+2][n+2];
        optimalBinarySearchTree1(a, b, m, s, w);
        System.out.println("二叉搜索树最小平均路长为："+m[1][n]);
        backtrace1(s, n, 1, n, 0, "0");
    }
